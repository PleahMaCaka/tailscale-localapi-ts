...
function backend(routes: Parameters<typeof stubFetch>[0]) {
  const stub = stubFetch(routes)
  restore = stub.restore
  calls = stub.calls

  return tailscale({ tailnet: "example.com", apiKey: "tskey" })
}
...
describe("tailscale keys", () => {
  ...
  test("revokes by key id", async () => {
    await backend({ "DELETE /tailnet/example.com/keys/k9": {} }).keys.revoke({
      id: "k9",
      secret: null,
      reusable: false,
      ephemeral: false,
