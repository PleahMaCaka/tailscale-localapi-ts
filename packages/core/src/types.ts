...
export interface TailnetNode {
  ...
  name: string
  /** Fully qualified MagicDNS name, when the control plane reports one. */
  fqdn: string
  ...
  /** The control plane's own representation, for anything this shape drops. */
  raw: unknown
}
...
export interface PolicyDocument {
  ...
  /** Concurrency token, when the control plane issues one. */
  version: string | null
}
